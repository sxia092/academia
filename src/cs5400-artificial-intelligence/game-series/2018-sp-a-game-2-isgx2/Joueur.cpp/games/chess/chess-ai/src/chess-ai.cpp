...
Action ChessAI::depthLimitedMinimax(const int depthLimit, const State& state, const PerceptSequence& history) {
    auto friendlyColor = state.colorAtPlay;
    auto possibleActions = actions(state);

    auto bestAction = possibleActions.back();
    auto currentMaxValue = minValue(depthLimit - 1, result(state, bestAction), friendlyColor, history);

    possibleActions.pop_back();

    for (const auto& action : possibleActions) {
        auto value = minValue(depthLimit - 1, result(state, action), friendlyColor, history);
...
float ChessAI::maxValue(const int depthLimit, const State& state, const Color& color, const PerceptSequence& history) {
    ...
    for (const auto& action : possibleActions) {
        ...
        value = std::max(value, minValue(depthLimit - 1, result(state, action),  color, newHistory));
    }
...
float ChessAI::minValue(const int depthLimit, const State& state, const Color& color, const PerceptSequence& history) {
    ...
    for (const auto& action : possibleActions) {
        ...
        value = std::min(value, maxValue(depthLimit - 1, result(state, action), color, newHistory));
    }
