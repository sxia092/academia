...
then
	...
	echo -e "\n\t-me, -move-eval:\n\t\tCheck individual moves outputted in generated files for any discrepencies against a precompiled stockfish move generator. It is required that the file output flag be used." | fmt | sed '2,$s/^/ /'
	exit
...
do
	case "$arg" in
		...
		-fo|-file-output)
			...
			if [[ "$@" =~ "-s=3" ]] || [[ "$@" =~ "-silent=3" ]]
			then
				echo "You cannot use the file output flag in conjunction with the silence all flag."
				exit
			fi
			FILEOUT="&>> $PWD/chess-"
			;;
		...
		-me|-move-eval)
			...
			if [ ! -f moveGen ]; then
				echo "File moveGen not found and is required with use of the move evaluation flag."
				exit
			fi
			#Check the validity of the argument
			if ! [[ "$@" =~ "-fo" ]] && ! [[ "$@" =~ "-file-output" ]]
...
then
	...
	then
		...
		then
			...
			exit
		elif [ ! -f "$PWD/target/java-client-jar-with-dependencies.jar" ] 
		then
			echo "java-client-jar-with-dependencies.jar does not exist in the target directory! Exiting..."
			exit
		elif [[ "$OCPREFIX" == "java" ]] && [ ! -e "$OTHERCLIENT/target" ]
		then
			echo "Target directory does not exist! Exiting..."
			exit
		elif [[ "$OCPREFIX" == "java" ]] && [ ! -f "$OTHERCLIENT/target/java-client-jar-with-dependencies.jar" ] 
		then
			echo "java-client-jar-with-dependencies.jar does not exist in the target directory! Exiting..."
			exit
		fi
		...
		then
			...
			exit
		elif [ ! -f "$PWD/build/$PREFIX-client" ]
		then
			echo "$PREFIX-client does not exist in the build directory! Exiting..."
			exit
		elif [[ -n "$OC" ]] && [[ ! -e "$OC/build" ]]
		then
			echo "Build directory does not exist! Exiting..."
			exit
		elif [[ -n "$OC" ]] && [[ ! -f "$OC/build/$OCPREFIX-client" ]]
		then
...
EOF
			...
			then
				cd $OTHERCLIENT
				(eval $TESTRUNPREFIXOC$TESTRUN "$SESSION$COUNTER $TESTRUNARGS" $FILEOUTFINAL $OUTPUT2 $PARALLEL)
				IDS+=("$!")
			else
				(eval $TESTRUNPREFIX$TESTRUN "$SESSION$COUNTER $TESTRUNARGS" $FILEOUTFINAL $OUTPUT2 $PARALLEL)
				IDS+=("$!")
			fi
		fi	
	done
...
then
	for log in chess-*.txt; do
		FILE="$(cat chess-1.txt | tr '\n' '\t' | tr -d '\r')"
		FENANDMOVES="$(echo "$FILE" | grep -Po 'fen=.*?fen=' | head -n 1 | sed -e 's/^fen=//' -e 's/fen=$//')"
		FAILS=()
		while [ -n "$FENANDMOVES" ]
		do
			MOVES="$(echo "$FENANDMOVES" | grep -Po '.*?\t' | tail -n +2 | grep -Po '[a-h][1-8]' | sed 'N;s/\n//')"
			FEN="$(echo "$FENANDMOVES" | grep -Po '.*?\t' | head -n 1)"
			FILE="$(echo "$FILE" | grep -Po 'fen=.*?\Kfen=.*')"
			
			STOCK="$(./moveGen "$FEN" 2>/dev/null)"
			if [ $? -ne 0 ]
			then
				SEEN=0
				for i in "${FAILS[@]}"
				...
				if [ $SEEN -ne 0 ]
				then
					echo "FAILED TO CHECK MOVES FOR STATE: $FEN"
					FAILS+=("$FEN")
					if [ -n "$STOCK" ]
					then
						echo "$STOCK"
				...
				F1="$(echo "$MOVES" | sort | tr -d ' ' | tr -d '\t' | tr -d '\r')"
				F2="$(echo "$STOCK" | sort | tr -d ' ' | tr -d '\t' | tr -d '\r')"
				DIFF="$(diff <(echo "$F1") <(echo "$F2")  | sed -n '1~2!p' | sed 's/>/MISSING:/' | sed 's/</EXTRA:/')"
				if [ -n "$DIFF" ]
				then
					echo "$FEN"
					echo "$DIFF"
	...
	echo "$ERRORS/$COUNTER turns contained errors"
fi
