import UIKit
import MapKit


class GameViewController: UIViewController, ItemPickerViewDelegate, GameDelegate, GameMenuViewDelegate
{
	var game:Game!
	
	private var lastCoord:CLLocationCoordinate2D!
	private var lastTouchTime:Date!
	private var lastIndex:Int!
	private var minimap:Minimap!
	private var map:GameMapView!
	private var menu:GameMenuView!
	
	// =================================================================================
	//							Touch Functions (debugging only)
	// =================================================================================
	
	override func touchesBegan(_ touches: Set<UITouch>, with event: UIEvent?)
	{
		guard let touchpoint = touches.first?.location(in:map) else
		{
			return
		}
		
		lastTouchTime = Date()
		lastCoord = map.convert(touchpoint, toCoordinateFrom:map)
		lastIndex = game.board.tileIndexForGPSLocation(location: lastCoord)
		
		if (lastIndex >= 0)
		{
			let updates = game.board.paintPoint(at:lastCoord, forPlayer:game.me!)
			game.addUpdates(updates:updates)
		}
	}
	
	// =================================================================================
	
	override func touchesMoved(_ touches: Set<UITouch>, with event: UIEvent?)
	{
		handleTouch(touch: touches.first)
	}
	
	// =================================================================================
	
	override func touchesEnded(_ touches: Set<UITouch>, with event: UIEvent?)
	{
		handleTouch(touch: touches.first)
	}
	
	// =================================================================================
	
	private func handleTouch(touch:UITouch?)
	{
		guard let touchpoint = touch?.location(in:map) else
		{
			return
		}
		
		let coord = map.convert(touchpoint, toCoordinateFrom:map)
		let index = game.board.tileIndexForGPSLocation(location: coord)
		let now = Date()
		
		// if at least one of the points is on the board, draw the line
		if (index >= 0) || (lastIndex >= 0)
		{
			if let me = game.me
			{
				let location = CLLocation(coordinate:coord, altitude:0, horizontalAccuracy:0, verticalAccuracy:0, course:0, speed:0, timestamp:now)
				game.move(player:me.id, to:location)
			}
		}
		
		lastCoord = coord
		lastTouchTime = now
		lastIndex = index
	}
	
	// =================================================================================
	//								    GameDelegate
	// =================================================================================
	
	func boardUpdated(tiles: [TileUpdate])
	{
		// the game model has been updated, so forward those updates to the UI components
		map.boardRenderer.updateTiles(tiles)
		minimap.updateTiles(tiles)
	}
	
	// =================================================================================
	
	func gameOver(won:Bool)
	{
		self.performSegue(withIdentifier:"GameToGameResults", sender:self)
	}
	
	// =================================================================================
	//								ItemPickerViewDelegate
	// =================================================================================
	
	func itemPicker(_ picker: ItemPickerView, pickedItem item: ItemType)
	{
		switch (item)
		{
			case .generic: game.me?.team = .red
			case .bomb: game.me?.team = .blue
			default: game.me?.team = .green
		}
	}
	
	// =================================================================================
	//								   GameMenuDelegate
	// =================================================================================
	
	func menuItemPressed(item: GameMenuItemType)
	{
		if (item == .quit)
		{
			if let vcs = self.navigationController?.viewControllers
			{
				let menu = vcs[vcs.count - 3]		// 2 vcs back from this
				
				game.quit()
				self.navigationController?.popToViewController(menu, animated:true)
			}
		}
		else if (item == .settings)
		{
			
		}
		else if (item == .toggleMinimap)
		{
			let current = UserDefaults.standard.bool(forKey:SETTING_MINIMAP_ON)
			minimap.isHidden = current			// not "!current" because isHidden is the opposite of is showing
			UserDefaults.standard.set(!current, forKey:SETTING_MINIMAP_ON)
		}
	}
	
	// =================================================================================
	//                                 UI Management
	// =================================================================================
	
	@objc func gameoverPressed(_:AnyObject)
	{
		game.endGame()
		gameOver(won:true)
	}
	
	// =================================================================================
	
	override func prepare(for segue: UIStoryboardSegue, sender: Any?)
	{
		GPSManager.defaultManager().turnOff()
		
		if let resultsVC = segue.destination as? GameResultsViewController
		{
			resultsVC.game = game
			ServerAPI.getStatsForGame(game, onComplete:resultsVC.update)
		}
	}
	
	// =================================================================================
	
	private func createMinimap()
	{
		let MINIMAP_PADDING:CGFloat = 7
		let MAX_MINIMAP_SIZE:CGFloat = 120
		
		let minimapWidth:CGFloat
		let minimapHeight:CGFloat
		
		minimap = Minimap(newBoard:game.board)
		
		if let imageSize = minimap.image?.size
		{
			if (imageSize.width > imageSize.height)
			{
				minimapWidth = MAX_MINIMAP_SIZE
				minimapHeight = (imageSize.height / imageSize.width) * minimapWidth
			}
			else
			{
				minimapHeight = MAX_MINIMAP_SIZE
				minimapWidth = (imageSize.width / imageSize.height) * minimapHeight
			}
		}
		else
		{
			minimapWidth = 0
			minimapHeight = 0
		}
		
		minimap.frame = CGRect(x:self.view.bounds.size.width - (MINIMAP_PADDING + minimapWidth),
		                       y:MINIMAP_PADDING,
		                       width:minimapWidth,
		                       height:minimapHeight)
		minimap.isHidden = !UserDefaults.standard.bool(forKey:SETTING_MINIMAP_ON)
		self.view.addSubview(minimap)
	}
	
	// =================================================================================
	
	private func createMenu()
	{
		let PADDING:CGFloat = 0
		let WIDTH:CGFloat = self.view.frame.size.width
		
		menu = GameMenuView(expandUpward:true, iconOnLeft:false)
		menu.delegate = self
		menu.frame.origin = CGPoint(x:WIDTH - (PADDING + menu.frame.size.width), y:self.view.frame.size.height - (PADDING + menu.frame.size.height))
		menu.updateUI()
		
		self.view.addSubview(menu)
	}
	
	// =================================================================================
	//                                 UIViewController
	// =================================================================================
	
    override func viewDidLoad()
	{
        super.viewDidLoad()
		
		game.delegate = self
		
		// create the map
		map = GameMapView(game:game)
		map.frame = CGRect(x:0, y:0, width:self.view.frame.size.width, height:self.view.frame.size.height)
		self.view.insertSubview(map, at:0)
		
		// create other UI components
		self.createMinimap()
		self.createMenu()
		
		// add the game over button
		let buttonSize = CGSize(width:200, height:50)
		let button = MyUtilities.buttonWithSize(size:buttonSize)
		button.setTitle("Game Over", for:.normal)
		button.addTarget(self, action:#selector(gameoverPressed(_:)), for:.touchUpInside)
		button.frame.origin = .zero
		self.view.addSubview(button)
    }

	// =================================================================================
	
	override var prefersStatusBarHidden: Bool
	{
		return true
	}
	
	// =================================================================================
}
