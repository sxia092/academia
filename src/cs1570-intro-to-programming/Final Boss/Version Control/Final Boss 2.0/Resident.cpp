...
unsigned Resident::filePlaceholder = 1;

Resident::Resident() {
    residentName = fileNameParsor();
    isPrice = residentName == "Clayton Price";
...
void Resident::randomAssignmentDNA() {
    ...
    residentForensics.DNA[LENGTH_OF_DNA] = NULL_TERMINATING_CHAR; // Ensure last char is terminating char.
...
void Resident::randomAssignmentFingerPrint() {
    ...
    for (int i = 0; i < HEIGHT_OF_FINGERPRINT_ARRAY; ++i) {
        ...
        residentForensics.forensicPrint.fingerPrintData[i][HEIGHT_OF_FINGERPRINT_ARRAY] = NULL_TERMINATING_CHAR;
    }
}
...
forensicData Resident::dropEvidence() {
    ...
    copyOfForensicsData.DNA[LENGTH_OF_DNA] = NULL_TERMINATING_CHAR;
    
    for (int i = 0; i < HEIGHT_OF_FINGERPRINT_ARRAY; ++i) {
        for (int j = 0; j < LENGTH_OF_FINGERPRINT_ARRAY; ++j) {
            copyOfForensicsData.forensicPrint.fingerPrintData[i][j] = residentForensics.forensicPrint.fingerPrintData[i][j];
        }
        copyOfForensicsData.forensicPrint.fingerPrintData[i][HEIGHT_OF_FINGERPRINT_ARRAY] = NULL_TERMINATING_CHAR;
    }
...
std::string Resident::fileNameParsor() {
    ...
    if (nameReadIn.is_open()) {
        gotoLine(nameReadIn, Resident::filePlaceholder);
        getline(nameReadIn, nameFromFile);
        Resident::filePlaceholder++;
    } else {
        errorMessage(2);
    }
...
std::string Resident::getRandomWeapon() {
    ...
    return isPrice ? "his Hammer of GNU and Knowledge of C++" : randomWeapon;
}
