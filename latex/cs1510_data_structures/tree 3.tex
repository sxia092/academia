...
\item If \textit{n} is a node and $T_1, T_2, T_3, \ldots , T_n$ are trees, then \textit{n} related to $T_1, T_2, \ldots, T_n$:
\begin{itemize}
    \item n is called the \underline{root}
    \item $T_1 \ldots T_n$ are called the subtrees of \textit{n}.
\end{itemize}
\item A \textit{path} is a sequence of nodes. $<a_1, a_2, a_3, \ldots, a_n>$ when $a_{i+1}$ is a parent of $a_i$ for $0 \leq i < n$.
...
\end{itemize}
\item[descendant/ancestor] If there is a path from node \textit{a} to node \textit{z} then \textit{a} is called a descendant of \textit{z}. \textit{z} is called an ancestor of \textit{a}. The root is every node's ancestor.
\end{itemize}
...
*/
    ...
    if (x < t -> m_data) {
        ...
        if (t -> m_right == nullptr && t -> m_left == nullptr) {
            ...
            // Now, temporary points to the child
            delete x;
            t = temporary;
        } else {
            // X has two children
...
index at \textit{i}, right(\textit{i}) = $2i + 1$, left(\textit{i}) = $2i + 2$. The parent of index $i$ is $\frac{i - 1}{2}$
