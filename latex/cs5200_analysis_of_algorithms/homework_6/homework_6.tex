...
\problem{}
Recall Strassen's algorithm belongs to the complexity class $\bigTheta{n^{\lg 7}}$. By using the Case~1 of the Main Theorem, we must find a that solves the inequality $\log _4 a < \lg 7$. The integer that solves the equation $\log _4 a = \lg 7$ is $49$; therefore,
...
\begin{enumerate}
    \item Main Theorem (\textit{Case \_num_3}), $T(n) \in \bigTheta{n^4}$
    \item Main Theorem (\textit{Case \_num_3}), $T(n) \in \bigTheta{n}$
    \item Main Theorem (\textit{Case \_num_2}), $T(n) \in \bigTheta{n^2 \lg n}$
    \item Main Theorem (\textit{Case \_num_3}), $T(n) \in \bigTheta{n^2}$
    \item Main Theorem (\textit{Case \_num_1}), $T(n) \in \bigTheta{n^{\log _2 7}}$
    \item Main Theorem (\textit{Case \_num_2}), $T(n) \in \bigTheta{\sqrt{n} \lg n}$
    ...
    Because $\log _1 1$ is undefined, we cannot use the Main Theorem; but we do not need it. We have $n^2$ work to do $n$ times. This implies that $T(n) \in \bigTheta {n^3}$.
\end{enumerate}


\problem{}
\begin{enumerate}
    \item Main Theorem (\textit{Case \_num_1}), $T(n) \in \bigTheta{n^{\log_3 4}}$
    ...
    \item Main Theorem (\textit{Case \_num_3}), $T(n) \in \bigTheta{n^{2.5}}$.
    \item We augment the equation to be $3T(n/3) + n/2$, where we can use Main Theorem (\textit{Case \_num_2}), $T(n) \in \bigTheta{n \lg n}$.
    \item With similar reasoning as in Step \_num_2, we use a recursion tree to get the results $T(n) \in \bigTheta{n \lg \lg n}$
