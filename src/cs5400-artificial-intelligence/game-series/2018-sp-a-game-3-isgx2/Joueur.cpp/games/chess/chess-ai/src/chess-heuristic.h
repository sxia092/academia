...
namespace ChessEngine {
    ...
    T ChessAIHeuristic<T>::materialAdvantage(const State& state, const Color& playerColor) {
        ...
        for (const auto& element : pieceWeights) {
            ...
            auto numberOfFriendlyPieces = playerColor == white ? state.whites[MoveEngine::pieceToInt(piece)].numberOfBits() : state.blacks[MoveEngine::pieceToInt(piece)].numberOfBits();
            auto numberOfEnemyPieces = playerColor == white ? state.blacks[MoveEngine::pieceToInt(piece)].numberOfBits() : state.whites[MoveEngine::pieceToInt(piece)].numberOfBits();
