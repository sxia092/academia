...
/--------------------Optionals--------------------/
 ...
 - Mr. Price is invincible to attacks. He automatically kills anyone trying to attack him.
...
int main() {
    ...
    do {
        ...
        pricesOffice.meeting(pricesStudents[residentOne], pricesStudents[residentTwo]);
        murderOccurred = pricesOffice.checkRoom();
        meetingResults(pricesOffice, MEETING_TIMES[timeCounter], murderOccurred, pricesStudents[residentOne], pricesStudents[residentTwo]);

        if (murderOccurred) {
            takeOutVictim(pricesStudents, pricesOffice, studentsRemaining, residentOne, residentTwo);
            csiAgent mainAgent;
