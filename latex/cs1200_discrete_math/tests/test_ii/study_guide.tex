...
Symbolically,
\begin{equation*}
F: X \rightarrow Y \text{is one-to-one} \quad \Leftrightarrow \quad \forall x_1, x_2 \in X, \ \text{if} \ F(x_1) = F(x_2) \ \text{then} \ x_1 = x_2
\end{equation*}

This can be read as: A function $F: X \rightarrow Y$ is \textit{not} one-to-one \textbf{if, and only if,} 
there exist elements $x_1, x_2 \in X$ with $F(x_1) = F(x_2)$ and $x_1 \neq x_2$.
