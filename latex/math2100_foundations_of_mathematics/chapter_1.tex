...
% Theorem _num_1: If M is a level set and a level B is above every level of M then M has a top level or there is a bottom level of the level set of all the levels that are above every level in M.
\section*{Theorem \_num_1}
\begin{theorem}
...
\begin{proof}
    ...
    \begin{description}
        \item[Case \_num_1] $D$ does not have a top level and $M$ does, and $D$ does have a top level and level set $M$ does not. A level set, by our definition of top level, cannot have both a top level and no top level at the same time.

        \item[Case \_num_2] $D$ does have a top level $T$ and $M$ does not.
        ...
        \item[Case \_num_3] $M$ does have a top level $T$ and $D$ does not. In this instance, see Case 2. If $M$ has a top level, then $D$ must include that top level $T$ and no levels above $T$.
    \end{description}
