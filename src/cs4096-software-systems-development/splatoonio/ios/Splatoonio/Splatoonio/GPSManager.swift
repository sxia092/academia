...
class GPSManager : NSObject, CLLocationManagerDelegate
{
	...
	// For the interpolated heading and location, call location() and heading() functions 
	...
	//									Lifecycle
	// =================================================================================
	// Returns an alert controller to be displayed if we can't use location services (failed to turn on), else returns nil
	func turnOn() -> UIAlertController?
	{
		if let alert = GPSManager.canUseLocationServices()
		{
			return alert
		}
		
		print("STATUS: turning GPSManager on...")
	...
	func locationManager(_ manager: CLLocationManager, didUpdateLocations locations: [CLLocation])
	{
		...
		// Currently we just use the most recent location of all the updates
		prevLocation = currentLocation;
		currentLocation = locations[locations.count - 1]
		print("got new location \((currentLocation!.coordinate.latitude, currentLocation!.coordinate.longitude))")
	...
	func turnOff()
	{
		if (onCount > 0)
		{
			...
			print("STATUS: turning GPSManager off...")
		}
		else
		{
			print("ERROR: GPSManager: extraneous turn off")
		}
	}
	...
	//									Query Functions
	// =================================================================================
	// Returns an alert controller to be displayed if we can't use location services, else returns nil
	static func canUseLocationServices() -> UIAlertController?
	{
		// Make sure that location services are on
		guard (CLLocationManager.locationServicesEnabled()) else
		{
			print("ERROR: Location services disabled")
			let alert = UIAlertController(title:"\(APP_NAME) requires your location", message:"Turn on Location Services in Settings to play", preferredStyle:.alert)
			alert.addAction(UIAlertAction(title:"Okay", style:.cancel, handler:nil))
			
			return alert
		}
		
		// make sure we have permission to use the user's location
		guard (CLLocationManager.authorizationStatus() != CLAuthorizationStatus.denied) &&				// generic denial of location
				(CLLocationManager.authorizationStatus() != CLAuthorizationStatus.restricted) else		// parental controls
		{
			print("ERROR: Location services - permission denied")
			let alert = UIAlertController(title:"\(APP_NAME) requires your location", message:"Give \(APP_NAME) permission to use your location in Settings.", preferredStyle:.alert)
			alert.addAction(UIAlertAction(title:"Okay", style:.cancel, handler:nil))
			
			return alert
		}
		
		// location services are on and we have permission to use them -> good to go
		return nil
	}
	...
	func heading() -> CLLocationDirection!
	{
		...
		// This time difference was found through experiementation and might vary per device, but appears to work well enough
