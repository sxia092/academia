...
class Minimap: UIImageView
{
	...
	func updateTiles(_ updates: [TileUpdate])
	{
		// update the changed tiles in the image
	...
	private func setBoard(newBoard:GameBoard)
	{
		...
		let contextBuffer = UnsafeMutableRawPointer.allocate(byteCount:4 * board.width * board.height, alignment:MemoryLayout<UInt8>.alignment)
