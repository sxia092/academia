...
\begin{enumerate}[label=\alph*.]
    ...
    \item Valid Operation
    \item Valid Operation
    \item Integrity Constraint (Did not update \texttt{WORKS\_ON} or \texttt{DEPENDENT})
...
\section*{Question \#2}
Grader note: the rho ($\rho$) is too small in \LaTeX{} to be distinguished from a subscript, so instead $\Rho$ is used in its place.
