...
\subsection*{CS2500 Project 1: Knapsack Memory Manager}
Comparative analysis of Brute Force, Dynamic Programming, and Greedy approaches to the 0-1 knapsack problem, framed as a smartphone OS deciding which apps to swap to disk. Each app has a memory cost and a recovery cost; the goal is to free a target amount of memory while minimizing recovery cost. A twelve-hour empirical test campaign exposes where each algorithm breaks down: Brute Force fails at $n=50$, Dynamic Programming remains tractable to $n=1{,}000$, and Greedy is always fast but only optimal under sorted ratios. The empirical scaling curves are the actual deliverable.

\subsection*{CS2500 Project 2: Graph Analytics Suite}
Six classical metrics for social network graphs, implemented from scratch in C++: degree distribution, shortest-path distribution, graph diameter, closeness centrality, betweenness centrality via Floyd-Warshall path reconstruction, and community detection. The implementation produces CSV data that the LaTeX report renders as pgfplots curves --- empirical graph theory turning ``what does centrality measure'' into a number you can plot.
...
% CS2500 Project 1: Knapsack Memory Manager
\includeassignment{cs2500_algorithms/cs2500_project1/report_ii.pdf}{CS2500 Project 1: Knapsack}

% CS2500 Project 2: Graph Analytics Suite
\includeassignment{cs2500_algorithms/cs2500_project2/report_ii.pdf}{CS2500 Project 2: Graph Analytics}
