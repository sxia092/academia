...
void csiAgent::gatherInformation(CrimeRoom roomToGatherInformation) {
    ...
    do {
        ...
        if (theEvidence.DNA[randomLocation] != DNA_CORRUPTION_CHAR) {
            ...
            ++assignmentCounter;
        } else if (theEvidence.DNA[randomLocation] == DNA_CORRUPTION_CHAR) {
            theEvidence.DNA[randomLocation] = roomToGatherInformation.getRoomDNACharacter(randomLocation);
            ++assignmentCounter;
        }
...
bool csiAgent::perpSearch(Resident residentToSearch) {
    ...
    for (int i = 0; i < LENGTH_OF_FINGERPRINT_ARRAY; ++i) {
        for (int j = 0; j < HEIGHT_OF_FINGERPRINT_ARRAY; ++j) {
            if (residentToSearch.getResidentFingerPrintCharacter(i, j) == FINGERPRINT_MEMBERS[true] &&
    ...
    fingerPrintScore = MATCHING_STARS_MULTIPLIER * (static_cast<float>(matchingNumber) / NUMBER_OF_STARS);
    ...
    offByX = absolute(theEvidence.hair - residentToSearch.getResidentHair());
    switch (offByX) {
        case (0):
