...
std::string readInputFile(const std::string& filename) {
    ...
    auto DIGIT         = R"(\d+)";
    auto SIGNED_DIGIT  = R"(-?\d+)";
    auto SPACE         = R"(\s)";
    auto NEWLINE       = R"(\n)";

    stringStream << DIGIT << NEWLINE << NEWLINE << "((" << SIGNED_DIGIT << SPACE << "+)+(" << SIGNED_DIGIT << "?" << NEWLINE << ")+)+";
...
*   @post See `return`.
**/
std::pair<SquareMatrix<double>, Vector<double>> parseFileContents(const std::string& fileContents) {
    ...
    auto aMatrix = SquareMatrix<double>(dimension);
