...
\newcommand{\presentcount}[1]{\addtocounter{_num_1}{1}\Roman{_num_1}}
...
\begin{frame}[plain,c]
    \centering {\usebeamerfont*{frametitle} \usebeamercolor[fg]{frametitle}\fontsize{40}{50}\selectfont \textit{_num_1}}
\end{frame}
}
\begin{document}
\begin{frame}
...
\begin{frame}
    \frametitle{Getting Started}
    If you would like to follow along, navigate to \url{https://github.com/IllyaStarikov} \textrightarrow \ repositories \textrightarrow \ msat_git_tutorial \textrightarrow \ clone or download\footnote{\includegraphics[width=0.5\textwidth]{github_clone}}. From here, you can either download as you normally would, or you use the super-cool terminal way:

    \begin{enumerate}
        \item Copy the link provided (something like \url{https://github.com/IllyaStarikov/msat_git_tutorial.git}).
        \item Open your terminal emulator of choice.
        \item Navigate to desired directory (using \shellcmd{cd DIRECTORY})
        \item Type in \shellcmd{git clone LINK}, where link is the copied link from Step \_num_1.
    \end{enumerate}
...
\begin{frame}
	...
	\begin{tikzpicture}[->,>=stealth',shorten >=1pt,auto,node distance=2cm,
		...
		\only<1>{%
			\node[draw=none] (M) [left of=A] {main};
			\path (M) edge (A);
		}

		\only<2>{%
			\node[draw=none] (M) [left of=B] {main};
			\path(M) edge (B);
		}

		\only<3>{%
			\node[draw=none] (M) [left of=C] {main};
			\path(M) edge (C);
		}
...
\begin{frame}
    ...
    \begin{multicols}{2}
        ...
        \begin{center}
            \includegraphics[width=0.45\textwidth]{xkcd_2}
        \end{center}
    \end{multicols}
\end{frame}
...
\begin{frame}
	...
	\begin{tikzpicture}[->,>=stealth',shorten >=1pt,auto,node distance=2cm,
		...
		\node[draw=none] (M) [above left= 0.05cm and 1cm of C] {main};
		\path(M) edge (C);
...
\begin{frame}
	...
	\only<2>{\shellcmd{git checkout main}}
...
\begin{frame}
	...
	\only<4>{\shellcmd{git merge main}\\ \shellcmd{Nothing to do!} \vspace{-1.2em}}
	\only<5>{\shellcmd{git checkout main}}
	...
	\begin{tikzpicture}[->,>=stealth',shorten >=1pt,auto,node distance=2cm,
		...
		\only<6>{%
			\node[draw=none] (M) [above left= 0.05cm and 1cm of Y] {main};
			\path(M) edge (Y);
		}
...
\begin{frame}
    ...
    When working with larger repositories, you might not necessarily have rights to arbitrarily merge --- you have to create a \textit{merge request}. A merge request allows for someone to review the changes being made to the main branch. \textit{Because merge requests are different per hosting service, this will be shown in the demo.}
\end{frame}
...
\begin{frame}
    ...
    \begin{center}
        \begin{tikzpicture}
            ...
            \draw[thick,->] (.3,0) -- (7.7,0) node[midway,fill=white] {\shellcmd{main}};
