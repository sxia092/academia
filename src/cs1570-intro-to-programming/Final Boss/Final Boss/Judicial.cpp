...
{
  m_name = judgeName;
  m_convictionRate = judgeConvictionRating;
  announceTheJudge();
}

bool Judicial::judgement(const std::string nameOfPrisoner)
{
  bool isConvicted = randomArbitrary(0, 100) <= m_convictionRate;
...
void Judicial::outcomeAnnouncement(const bool convictedToDeath,
                   const std::string nameOfPrisoner)const
{
  ...
  if (convictedToDeath == false)
  {
    switch (response)
    {
      case 0:
        std::cout
        << "A good lawyer knows the law." << std::endl
      ...
      case 1:
        std::cout
        << "Off the hook, even though there was a good "
        << "change he/she did it" << std::endl
        << "Haters gonna hate"<< std::endl;
        break;

      case 2:
        std::cout
        << "99 Problems. But his/her conviction isn't one. "
        << "Got away squeaky clean" << std::endl
        << "At least it isn't JavaScript. "
        << "Then it would be 99.0004 problems." << std::endl;
        break;
  ...
  else if (convictedToDeath == true)
  {
    switch (response)
    {
      ...
      case 2:
        std::cout
        << nameOfPrisoner
        << " has been sentenced to death by exile."
        << std::endl;
        break;
...
void Judicial::announceTheJudge()const
{
  std::cout
