...
void setTheScene() {
    cout << endl << endl << "/*-------------------- The Beginning --------------------*/" << endl;
    string storySegment;
    ifstream fin;
    fin.open("Backstory.txt");
...
void meetingResults(const CrimeRoom roomOfCrime, const string meetingTime, const bool murderOccured, Resident residentOne, Resident residentTwo) {
    cout << meetingTime << "\t|\t";
    if (murderOccured) {
...
void takeOutVictim(Resident mainResidents[], CrimeRoom roomOfCrime, int & SizeOfArray, int locationOfFirstResident, int locationOfSecondResident) {
    if (roomOfCrime.getDeadResident() == mainResidents[locationOfFirstResident].getResidentName()) {
...
void investigation(csiAgent agentForInvestigation, Resident residentsToSearch[], int & sizeOfResidents, CrimeRoom roomToSearch) {
    ...
    matchFound ?
    cout << endl << endl << "Match Found! After " << roomVisits << " visits it was identified to be " << residentsToSearch[residentPlace].getResidentName() << " with " << residentsToSearch[residentPlace].getWeapon() << "!" << endl << endl :
    cout << endl << endl << "No match found:(" << endl << "Better luck next time..." << endl << endl;
...
void judgementDay(Resident residentArray[], int & sizeOfArray, int locationOfJudgingResident) {
    ...
    bool conviction = theJudge.judgement(residentArray[locationOfJudgingResident].getResidentName());
    if (conviction) {
        removeResidentFromArray(residentArray, sizeOfArray, locationOfJudgingResident);
    }
...
void closingStatement(const Resident residentZero, const bool someoneDidWin) {
    ...
    cout << endl << "It's the end of the semester, and everyone is dead. All have been killed, turned to be murders, sent to jail, free to walk the streets." << endl << endl;
    cout << "All except you";
    if (someoneDidWin) {
        cout << " and " << residentZero.getResidentName() << ", who is now in permanant psychiatric evaluation";
    }
    cout << "." << endl << endl;
...
void timeManagement(int & timeOfDay, int & theDay, const bool isMurderOccuring) {
    if (timeOfDay >= MEETINGS_PER_DAY - 1 || isMurderOccuring) {
        timeOfDay = 0;
        theDay++;
        cout<< endl << "/*-------------------- Day " << theDay  << " --------------------*/" << endl;
    } else {
        ++timeOfDay;
    }
    return;
...
int absolute(const int numberToAbsolute) {
    return (numberToAbsolute < 0 ) ? (numberToAbsolute * - 1) : numberToAbsolute;
}
