    ...
    for (const auto& characterAtIndex : token) {
        ...
        if (isdigit(characterAtIndex)) {
            ...
            if (result.first == white) {
                whiteBitboard[ChessEngine::pieceToInt(result.second)] |= Bitboard().fromIndex(bitboardIndex);
            } else if (result.first == black) {
                blackBitboard[ChessEngine::pieceToInt(result.second)] |= Bitboard().fromIndex(bitboardIndex);
            } else {
...
State FenParser::operator()(const std::string &string) {
    ...
    auto allWhites = ChessEngine::allBitboardsInOneBoard(whiteBitboards), allBlacks = ChessEngine::allBitboardsInOneBoard(blackBitboards);
